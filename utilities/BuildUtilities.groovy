	 ...
	 buildList.each { String buildFile ->
	     ...
	     if (loadLanguageConfigurationProperties && loadLanguageConfigurationProperties.toBoolean()) {
	         ...
	         if (languageConfigurationPropertyFileName != null) {
	             ...
	             // String languageConfigurationPropertyFilePath = getAbsolutePath(props.application) + "/${propertyFilePath}/${languageConfigurationPropertyFileName}.${propertyExtention}"                    
	     ...
	     filePropMap.each { entry ->
	         ...
	         (filePatternIsMapped, filePatternIsMappedAtFileName, noChangeFilePattern) = checkExistingFilesPropertyDefinition(buildFile, member, entry.key)
	         ...
	         if (filePatternIsMapped ) {
	            if (filePatternIsMappedAtFileName) {
	                ...
	                if (props.verbose) println("    *! Warning: $buildFile is already mapped as a file pattern as part of a wildcard mapping in file.properties.")
	                if (props.verbose) println("    *! Warning: Please check the backup list above and fix it to fetch the correct file properties.")
	            }
	         } else {
	             // Add the buildFile file pattern with new value    
	             props.addFilePattern(entry.key, entry.value, buildFile)
...
def loadProgramTypeProperties(String languageConfigurationPropertyFileName, String languageConfigurationPropertyFile, String buildFile) {
    ...
    languageConfigProps.entrySet().each { entry ->
       if (props.verbose) println "    Found langugae configuration property ${entry.key} = ${entry.value} for $buildFile"
       filePropMap[entry.key] = entry.value
    }
    return filePropMap
}
...
def checkExistingFilesPropertyDefinition(String buildFile, String member, String entryKey){
    ...
    if (props.verbose) println "    Checking existing file properties for ${entryKey}"
    ...
    return [filePatternIsMapped, filePatternIsMappedAtFileName, noChangeFilePattern]
}