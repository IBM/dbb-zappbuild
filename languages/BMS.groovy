...
sortedList.each { buildFile ->
	...
	buildUtils.copySourceFiles(buildFile, props.bms_srcPDS, null, null, null, null)
...
def createCopyGenCommand(String buildFile, String member, File logFile) {
	...
	compile.dd(new DDStatement().name("SYSPUNCH").dsn("${props.bms_cpyPDS}($member)").options('shr').output(true).deployType(props.bms_copy_deployType))
	[1,2,3].each { num ->
